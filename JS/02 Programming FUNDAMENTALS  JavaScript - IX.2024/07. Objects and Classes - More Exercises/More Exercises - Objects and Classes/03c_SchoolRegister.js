function schoolRegister (inputArr) {
    ...
    for (let el of inputArr) {
        ...
        if (score >= 3) {
            ...
            if (!register[nextGrade]) {
                register[nextGrade] = { students: [], totalScore: 0 }
            }

            let student = new Student(name, grade, score)
            register[nextGrade].students.push(student)
    ...
    for (let el of sortedGrades) {
        ...
        let students = register[el].students.map(s => s.name).join(', ')
        let averageScore = (register[el].totalScore / register[el].students.length).toFixed(2)
